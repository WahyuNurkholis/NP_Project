...
fig.text(0.075, 0.965, 'SPAD vs laboratory chlorophyll, fitted separately by shade block', fontsize=25, fontweight='bold', va='top')
fig.text(0.075, 0.917, 'Each point is one sample (n = 270 = 54 samples × 5 harvest cycles). SPAD is the whole-leaf mean. One linear fit per block (n = 135 each).',
...
fig.savefig(FIGS / 'spad_vs_chl_by_shade.svg', facecolor=SURF)
if os.environ.get('PREVIEW_DIR'): fig.savefig(os.path.join(os.environ['PREVIEW_DIR'], 'spad_vs_chl_by_shade.png'), dpi=int(os.environ.get('PREVIEW_DPI', 100)), facecolor=SURF)
