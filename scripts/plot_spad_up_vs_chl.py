...
import re, openpyxl
# SPAD = mean of the 'up' leaf-part readings only (row 1 header == 'up'), averaged over the readings present
spad = {}
...
fig.text(0.075, 0.965, 'Upper-leaf SPAD vs laboratory chlorophyll, fitted separately by shade block', fontsize=25, fontweight='bold', va='top')
fig.text(0.075, 0.917, 'Each point is one sample (n = 270 = 54 samples × 5 harvest cycles). SPAD is the mean of the upper-leaf readings only. One linear fit per block (n = 135 each).',
...
for k, (name, y) in enumerate(Y.items()):
    ...
    ax.set_xlabel('Upper-leaf SPAD value', fontsize=16, labelpad=6)
...
ax.set_title('Pearson correlation (r) between upper-leaf SPAD and laboratory chlorophyll, by subset', loc='left',
...
fig.text(0.11, 0.062,
         ...
         f'SPAD source: SPAD_Data_B3_dropped_per_part.xlsx, up columns.',
         fontsize=14, color=INK2, va='top')
fig.text(0.11, 0.028, 'Mean taken over the readings present in each row (up to 4 in cycle 1 and 6 in cycles 2–5; at least 2). Font: Nimbus Roman (Times-compatible).',
         fontsize=13.5, color=INK2, va='top', style='italic')
fig.savefig(FIGS / 'spad_up_vs_chl_by_shade.svg', facecolor=SURF)
if os.environ.get('PREVIEW_DIR'): fig.savefig(os.path.join(os.environ['PREVIEW_DIR'], 'spad_up_vs_chl_by_shade.png'), dpi=int(os.environ.get('PREVIEW_DPI', 100)), facecolor=SURF)
