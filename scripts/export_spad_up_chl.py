"""Upper-leaf SPAD (mean of the 'up' columns in SPAD_Data_B3_dropped_per_part.xlsx) linked to the lab chlorophyll data.

Writes results/tables/SPAD_Up_vs_Chl_Lab.xlsx with two sheets:
  Data        one row per sample and harvest cycle: up SPAD readings, their mean, and the lab values
  Regression  linear fit of each lab measure on up SPAD, for all samples, each block and each cycle
"""
import re
...
# up SPAD readings, kept per reading; mean over the readings present
up = {}
for cyc, ws in enumerate(openpyxl.load_workbook(DATA / 'backup_original' / 'SPAD_Data_B3_dropped_per_part.xlsx', data_only=True).worksheets, 1):
    cols = [j for j in range(3, ws.max_column + 1) if ws.cell(1, j).value == 'up']
    for r in range(3, ws.max_row + 1):
        if ws.cell(r, 2).value is not None:
            up[(cyc, ws.cell(r, 2).value)] = [ws.cell(r, j).value if isinstance(ws.cell(r, j).value, (int, float)) else None for j in cols]
...
for r in openpyxl.load_workbook(DATA / 'Chl_Lab_Data.xlsx', data_only=True).active.iter_rows(min_row=2, max_col=5, values_only=True):
    ...
    v = up[(int(r[0]), r[1])]; p = [x for x in v if x is not None]
    rows.append(dict(Harvest=int(r[0]), Sample=r[1], T=int(t), Block=int(b), up=v, n=len(p), SPAD=sum(p) / len(p),
                     A=r[2], B=r[3], Tot=r[4]))

wb = openpyxl.Workbook(); bold = Font(bold=True); ctr = Alignment(horizontal='center', vertical='center', wrap_text=True)
ws = wb.active; ws.title = 'Data'
nmax = max(len(d['up']) for d in rows)
hdr = ['Harvest', 'Sample', 'Treatment', 'Block'] + [f'Up SPAD {i}' for i in range(1, nmax + 1)] + \
      ['Readings used', 'Up SPAD mean', 'Lab Chl a', 'Lab Chl b', 'Lab Total Chl']
ws.append(hdr)
for d in rows:
    ws.append([d['Harvest'], d['Sample'], f"T{d['T']}", f"B{d['Block']}"] + d['up'] + [None] * (nmax - len(d['up'])) +
...
ws.append(['Linear regression: lab value = slope × up SPAD + intercept'])
ws['A1'].font = bold
...
out = TABLES / 'SPAD_Up_vs_Chl_Lab.xlsx'
wb.save(out)
