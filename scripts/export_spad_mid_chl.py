"""Mid-leaf SPAD (mean of the 'mid' columns in SPAD_Data_B3_dropped_per_part.xlsx) linked to the lab chlorophyll data.

Writes results/tables/SPAD_Mid_vs_Chl_Lab.xlsx with two sheets:
  Data        one row per sample and harvest cycle: mid SPAD readings, their mean, and the lab values
  Regression  linear fit of each lab measure on mid SPAD, for all samples, each block and each cycle
"""
import re
...
# mid SPAD readings, kept per reading; mean over the readings present
mid = {}
for cyc, ws in enumerate(openpyxl.load_workbook(DATA / 'backup_original' / 'SPAD_Data_B3_dropped_per_part.xlsx', data_only=True).worksheets, 1):
    ...
    for r in range(3, ws.max_row + 1):
        if ws.cell(r, 2).value is not None:
            mid[(cyc, ws.cell(r, 2).value)] = [ws.cell(r, j).value if isinstance(ws.cell(r, j).value, (int, float)) else None for j in cols]
...
for r in openpyxl.load_workbook(DATA / 'Chl_Lab_Data.xlsx', data_only=True).active.iter_rows(min_row=2, max_col=5, values_only=True):
    ...
    v = mid[(int(r[0]), r[1])]; p = [x for x in v if x is not None]
    rows.append(dict(Harvest=int(r[0]), Sample=r[1], T=int(t), Block=int(b), mid=v, n=len(p), SPAD=sum(p) / len(p),
                     A=r[2], B=r[3], Tot=r[4]))

wb = openpyxl.Workbook(); bold = Font(bold=True); ctr = Alignment(horizontal='center', vertical='center', wrap_text=True)
ws = wb.active; ws.title = 'Data'
nmax = max(len(d['mid']) for d in rows)
hdr = ['Harvest', 'Sample', 'Treatment', 'Block'] + [f'Mid SPAD {i}' for i in range(1, nmax + 1)] + \
      ['Readings used', 'Mid SPAD mean', 'Lab Chl a', 'Lab Chl b', 'Lab Total Chl']
ws.append(hdr)
for d in rows:
    ws.append([d['Harvest'], d['Sample'], f"T{d['T']}", f"B{d['Block']}"] + d['mid'] + [None] * (nmax - len(d['mid'])) +
...
ws.append(['Linear regression: lab value = slope × mid SPAD + intercept'])
ws['A1'].font = bold
...
out = TABLES / 'SPAD_Mid_vs_Chl_Lab.xlsx'
wb.save(out)
