...
import re, openpyxl
# SPAD = mean of the 'low' leaf-part readings only (row 1 header == 'low'), averaged over the readings present
spad = {}
...
fig.text(0.075, 0.965, 'Lower-leaf SPAD vs laboratory chlorophyll, fitted separately by shade block', fontsize=25, fontweight='bold', va='top')
fig.text(0.075, 0.917, 'Each point is one sample (n = 270 = 54 samples × 5 harvest cycles). SPAD is the mean of the lower-leaf readings only. One linear fit per block (n = 135 each).',
...
for k, (name, y) in enumerate(Y.items()):
    ...
    ax.set_xlabel('Lower-leaf SPAD value', fontsize=16, labelpad=6)
...
ax.set_title('Pearson correlation (r) between lower-leaf SPAD and laboratory chlorophyll, by subset', loc='left',
...
fig.text(0.11, 0.062,
         ...
         f'SPAD source: SPAD_Data_B3_dropped_per_part.xlsx, low columns.',
         fontsize=14, color=INK2, va='top')
fig.text(0.11, 0.028, 'Mean taken over the readings present in each row (4 in cycle 1, 6 in cycles 2–5; one cycle-4 row has 5). Font: Nimbus Roman (Times-compatible).',
         fontsize=13.5, color=INK2, va='top', style='italic')
fig.savefig(FIGS / 'spad_low_vs_chl_by_shade.svg', facecolor=SURF)
if os.environ.get('PREVIEW_DIR'): fig.savefig(os.path.join(os.environ['PREVIEW_DIR'], 'spad_low_vs_chl_by_shade.png'), dpi=int(os.environ.get('PREVIEW_DPI', 100)), facecolor=SURF)
